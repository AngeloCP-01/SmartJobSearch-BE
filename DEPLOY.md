...
3. Render will prompt for the env vars marked `sync: false`. Fill in:
   ...
   - **AI + RAG (needed for ATS "Use AI", cover letters, Tailor Résumé, and Draft-in-Editor).** Leave all AI vars blank to keep résumé analysis deterministic (those features degrade gracefully). To enable them:
     ...
     - `OPENROUTER_MODEL` → `nex-agi/nex-n2.5-pro:free,nex-agi/nex-n2.5-mini:free,google/gemma-4-31b-it:free`, with `OPENROUTER_REASONING_EFFORT=none`. Both are declared in `render.yaml`; update any existing dashboard overrides and deploy the response-validation fix together. Disabling reasoning reserves the output budget for the final answer. Free endpoints can still be rate-limited, including a shared daily cap. `EMBEDDING_MODEL` and `NVIDIA_BASE_URL` retain their separate blueprint defaults.
...
- **⚠️ Changing `EMBEDDING_MODEL` requires a full re-index:** embeddings from two different models are not comparable, even at the same width. Old vectors compared against new query vectors yield numerically valid but **semantically meaningless** scores — silently wrong search results, not errors. After any model change: deploy, delete every `DocumentChunk` row, then `POST /api/v1/rag/reindex` (authenticated, per user) to re-embed. The model must emit 1024-dim vectors to fit the column; `llama-nemotron-embed-vl-1b-v2` does so via the `dimensions` parameter.
