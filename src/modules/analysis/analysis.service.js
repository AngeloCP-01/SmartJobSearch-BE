...
const { coverLetterMessages, tailorMessages } = require('./engine/prompts');
...
async function generateCoverLetter(userId, { applicationId, documentId }) {
  ...
  let result;
  try {
    result = await generateTextWithFallback(coverLetterMessages({
      companyName, position, jd, resumeText,
    }));
  } catch (err) {
...
async function generateTailoringSuggestions(userId, { applicationId, documentId }) {
  ...
  try {
    result = await generateJson(
      tailorMessages({ jd, resumeText, evidenceBlock }),
