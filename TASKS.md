...
> **Update (2026-09-25):** **Application askingSalary field + auto-set applicationDate on "Applied" — done, TDD, backend + frontend.** New nullable `Application.askingSalary` Int column (migration `20260925000000_add_asking_salary`) + Zod validation, alongside `salaryMin`/`salaryMax`. `updateStatus` and the generic `update` (both can change `status`) now share `withAppliedDate()`: sets `applicationDate` to now when status transitions to `Applied` and `applicationDate` isn't already set, never overwrites an existing one (existing `applicationDate` field is the "applied date" — no separate `appliedDate` column). No new API version. **BE 481/482 tests** (1 pre-existing parallel-DB flake, unrelated — passes in isolation). **Frontend** (`SmartJobSearchCRM-FE`): `ApplicationDrawer` gets an "Asking salary" field next to Min/Max, and the Status `<select>` auto-fills an empty Applied-date input with today's date when switched to `Applied` (mirrors the backend rule client-side for instant feedback), without overwriting an existing date. **FE 357/357 tests.**
